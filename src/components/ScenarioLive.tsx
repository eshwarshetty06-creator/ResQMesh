...
interface NodeLocation {
    ...
    vitals?: number;
    lastSeen: number;
    path: [number, number][]; // Breadcrumb history
}
...
export default function ScenarioLive({ onBack, initialData }: { onBack: () => void, initialData?: { role?: Role, bpm?: number, userName?: string, serverName?: string } }) {
    // --- STATE ---
    ...
    const [myPath, setMyPath] = useState<[number, number][]>([]); // Store my own breadcrumbs
    ...
    const generateOffer = async () => {
        ...
        // Removed setDirectOffer(sdp) as it's unused
        QRCode.toDataURL(sdp, { margin: 2, scale: 4, color: { dark: '#FCD34D', light: '#00000000' } })
            .then(url => setDirectQR(url));
    ...
    const generateAnswer = async () => {
        ...
        // Removed setDirectAnswer(ansSdp) as it's unused
        QRCode.toDataURL(ansSdp, { margin: 2, scale: 4, color: { dark: '#FCD34D', light: '#00000000' } })
            .then(url => setDirectQR(url));
    ...
    useEffect(() => {
        ...
        if (initialData?.role === 'responder') {
            setCustomId(initialData.serverName || initialData.userName || `SERVER-${Math.floor(Math.random() * 900) + 100}`);
        } else {
            setCustomId(initialData?.userName || `CIV-${Math.floor(Math.random() * 900) + 100}`);
        }

        if (initialData?.serverName) {
            setPasskey(initialData.serverName);
        }
    }, [initialData]);

    // --- AUTO CONNECTION ---
    useEffect(() => {
        // Only trigger auto-connect if we don't already have an ID and a customId has been generated
        if (myId || !customId) return;

        // If civilian doesn't have a passkey, they can still join the mesh and use discovery
        if (role === 'civilian' && !passkey.trim()) {
            console.warn('No passkey provided. Operating as isolated node.');
        }
        ...
        const autoConnectTimer = setTimeout(() => {
            ...
            const peer = new Peer(customId, {
                ...
                port: isLocal ? 9000 : (window.location.port ? parseInt(window.location.port) : 443),
                path: '/peerjs',
                secure: !isLocal && window.location.protocol === 'https:',
                config: {
                    iceServers: [
            ...
            peer.on('open', id => {
                ...
                // Auto-connect civilian to server
                if (role === 'civilian' && passkey) {
                    setTimeout(() => connectToPeer(passkey), 800);
                }
            });
        ...
        c.on('data', (data: any) => {
            ...
            if (s.startsWith('LOC:')) {
                ...
                setNodes(prev => {
                    ...
                    const newPath: [number, number][] = ([...existingPath, [newLat, newLng] as [number, number]] as [number, number][]).slice(-50); // Keep last 50 points
    ...
    const shareLocation = () => {
        navigator.geolocation.getCurrentPosition(pos => {
            ...
            setMyPath((prev: [number, number][]) => ([...prev, [lat, lng] as [number, number]] as [number, number][]).slice(-50)); // Trace my own path
        ...
        if (activeTab === 'map' && mapRef.current) {
            ...
            mapRef.current.eachLayer((l: any) => {
                if (l instanceof L.Marker || l instanceof L.Circle || l instanceof L.Polyline) {
                    // Do not remove the tile layer map base
                    if (!('_url' in l)) mapRef.current.removeLayer(l);
                }
            });

            // My location marker and breadcrumbs
            if (myLocation) {
                // Draw my path
                if (myPath.length > 1) {
                    L.polyline(myPath, { color: '#00f2fe', weight: 3, opacity: 0.6, dashArray: '5, 8' }).addTo(mapRef.current);
                }
            ...
            // Peer nodes & Breadcrumbs
            Object.values(nodes).forEach(n => {
                const statusClass = n.status.toLowerCase();

                // Draw peer path
                if (n.path && n.path.length > 1) {
                    const lineColor = n.role === 'responder' ? '#4facfe' : (statusClass === 'critical' ? '#ff0055' : '#00ff9d');
                    L.polyline(n.path, { color: lineColor, weight: 3, opacity: 0.5, dashArray: '5, 8' }).addTo(mapRef.current);
                }
    ...
    }, [nodes, myLocation, activeTab, myPath]);
                    ...
                    {!myId ? (
                        <div className="boot-sequence">
                            <div className="boot-card glass">
                                ...
                                <p className="boot-step">DEVICE PREPARED: {customId}</p>
                                <p style={{ fontSize: '0.65rem', color: '#64748B', marginBottom: '20px', textAlign: 'center', textTransform: 'uppercase' }}>
                                    {role === 'responder' ? 'Creating Server Node...' : `Connecting to: ${passkey || 'WAITING FOR PASSKEY...'}`}
                                </p>
                            </div>
                        </div>
                    ) : (
