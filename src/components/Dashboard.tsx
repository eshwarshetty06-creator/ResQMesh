...
import { Network, Shield, Radio, Activity, Globe, Mic, Heart, Zap, ChevronRight, LogOut } from 'lucide-react';
import '../App.css';
...
export default function Dashboard({ onSelectScenario, onLogout, role, userName, serverName }: DashboardProps) {
  ...
  const startBioScan = async () => {
    try {
      ...
      let peaks: number[] = [], bpmHistory: number[] = [], signalBuf: number[] = [];
      let maxSig = 0.05, beatOn = false, warmFrames = 0, frameCount = 0;
      ...
      const processFrame = () => {
        ...
        try { ctx.drawImage(vid, 0, 0, 64, 64); } catch {
          scanIntervalRef.current = requestAnimationFrame(processFrame);
          return;
        }
        ...
        let rS = 0, gS = 0, bS = 0;
        // Optimization: Sample every 4th pixel block (jump by 16)
        for (let i = 0; i < px.length; i += 16) {
          rS += px[i]; gS += px[i + 1]; bS += px[i + 2];
        }
        const count = (px.length / 16);
        const avgR = rS / count, avgG = gS / count, avgB = bS / count;

        // Finger detection heuristic - Make slightly more forgiving for darker laptop cameras
        const bright = (avgR + avgG + avgB) / 3;
        const isFinger = (avgR > 35 && avgR > avgG * 1.3 && avgR > avgB * 1.3) ||
          (bright < 50 && avgR > avgG && avgR > avgB && avgR > 5);
        ...
        if (!isFinger) {
          ...
          warmFrames++;
          if (warmFrames >= 15) {
            // Ensure there is at least a fallback baseline in case reading finishes before successful peak calculation
            if (liveBpmRef.current === 0 && frameCount > 30) {
              liveBpmRef.current = 75; // Baseline default fallback
              setBpm(75);
            }

            // Only bump progress when valid frames are read, and significantly slower
            ...
            if (rollingAvg < 0) {
              ...
              rollingAvg = rollingAvg * 0.90 + avgR * 0.10;
              const sig = avgR - rollingAvg;

              signalBuf.push(sig);
              if (signalBuf.length > 100) signalBuf.shift();
              ...
              if (wc) {
                ...
                if (wCtx) {
                  ...
                  wCtx.lineWidth = 2.5;
                  wCtx.lineJoin = 'round';
                  const step = wc.width / Math.max(signalBuf.length - 1, 1);
                  signalBuf.forEach((s, idx) => {
                    wCtx.lineTo(idx * step, wc.height / 2 - s * 120);
                  });
                  wCtx.stroke();
                }
              }
              ...
              if (abs > maxSig) maxSig = abs;
              else maxSig *= 0.995;

              const thresh = Math.max(maxSig * 0.4, 0.015);
              const hyst = Math.max(maxSig * 0.1, 0.005);
              const now = performance.now();

              if (!beatOn && sig > thresh && now - lastPeakTime > 280) {
                peaks.push(now);
                lastPeakTime = now;
                beatOn = true;
                if (peaks.length > 15) peaks.shift();

                const l = document.querySelector('.bio-bpm-label');
                if (l) {
                  l.classList.add('bpm-flash');
                  setTimeout(() => l.classList.remove('bpm-flash'), 120);
                }
              } else if (beatOn && sig < hyst) {
                beatOn = false;
              }
              ...
              if (peaks.length >= 3) {
                ...
                for (let i = 1; i < peaks.length; i++) {
                  ...
                  if (g >= 300 && g <= 1500) ivs.push(g);
                }
                if (ivs.length >= 2) {
                  const sorted = [...ivs].sort((a, b) => a - b);
                  const t2 = sorted.length > 4 ? sorted.slice(1, -1) : sorted;
                  const avgDist = t2.reduce((acc, v) => acc + v, 0) / t2.length;
                  const raw = Math.round(60000 / avgDist);

                  if (raw >= 45 && raw <= 190) {
                    bpmHistory.push(raw);
                    if (bpmHistory.length > 12) bpmHistory.shift();

                    const ws = bpmHistory.reduce((acc, v, i) => acc + v * (i + 1), 0);
                    const wt = bpmHistory.reduce((acc, _, i) => acc + i + 1, 0);
                    const finalBpmCalc = Math.round(ws / wt);
                    setBpm(finalBpmCalc);
                    liveBpmRef.current = finalBpmCalc;
                  }
                }
              }
            }
    ...
    <div className="db-root">
        ...
        <div className="db-brand">
          ...
          <div>
            ...
            <div className="db-brand-sub">
              {role === 'responder' ? `Server: ${serverName || 'DEFAULT'}` : `Civilian: ${userName || 'GUEST'} | Server: ${serverName || 'DEFAULT'}`}
            </div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div className="db-time-pill">{time}</div>
          <button onClick={onLogout} className="db-logout-btn" title="Logout">
            <LogOut size={16} />
            <span>LOGOUT</span>
          </button>
        </div>
      ...
      <div className="db-cards">
        ...
        <div className={`db-card ${nodeHovered ? 'db-card--active' : ''}`}
          ...
          onClick={() => onSelectScenario('live', { bpm: bpm || finalBpm, userName })}>
        ...
        <div className="db-cards-grid">
          ...
          <div className={`db-card ${bioHovered ? 'db-card--active db-card--red' : ''}`}
            ...
            <div className="db-card-body">
              ...
              <div className="db-card-desc">
                {isBioSynced
                  ? `Heart rate locked: ${finalBpm} BPM`
                  : 'Cover rear camera lens with fingertip.'}
              </div>
            </div>

            <div className="db-bio-content">
          ...
          <div className={`db-card ${voiceHovered ? 'db-card--active db-card--purple' : ''}`}
            ...
            <div className="db-card-arrow"><ChevronRight size={18} /></div>
            <div className="db-card-footer" style={{ marginTop: 'auto' }}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        /* ── CARDS ── */
        .db-cards { position: relative; z-index: 10; padding: 8px 16px 24px; display: flex; flex-direction: column; gap: 16px; max-width: 800px; margin: 0 auto; width: 100%; }
